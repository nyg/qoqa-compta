...
- Charts use Recharts (`ComposedChart` with bar + line dual-axis; pie chart for spending breakdown); the date range picker uses react-day-picker through `src/views/components/ui/calendar.tsx`. Its month/year caption replaces react-day-picker's native `<select>` with a Base UI `Select`, because a WebView hands the OS popup menu a font it cannot resolve and falls back to a serif. Three things there are load-bearing: the trigger is `relative` so it paints above the absolutely positioned `nav` and can be clicked, the popup renders inline rather than through `Select.Portal` so the picker's own click-outside handler still sees it as inside, and `positionMethod="fixed"` keeps Base UI off its default `offsetParent` maths, which WebKit resolves differently and which parked the popup far to the right for a frame, leaving the whole page scrolled there. `modal={false}` is not needed for correctness, only to keep a caption dropdown from locking page scroll
