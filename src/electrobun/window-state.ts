...
function statePath(): string {
  return path.join(resolveConfigDir(), STATE_FILE);
}
...
function displayFingerprint(): string {
  try {
    return Screen.getAllDisplays()
      .map(
        (display) =>
          `${display.id}:${display.bounds.x},${display.bounds.y},${display.bounds.width}x${display.bounds.height}`
      )
      .sort()
      .join(";");
  } catch {
...
function writeState(state: PersistedWindowState): void {
  ...
  if (lastWritten === null) {
    try {
      lastWritten = fs.readFileSync(statePath(), "utf-8");
    } catch {
      lastWritten = "";
    }
  }
...
function isSanePosition(bounds: WindowBounds): boolean {
  ...
  const spanY = Math.max(
    ...displays.map((display) => Math.abs(display.y) + display.height)
  );
  const horizontallyVisible =
    bounds.x + bounds.width - MIN_VISIBLE > minX &&
    bounds.x + MIN_VISIBLE < maxX;
  const verticallyPlausible =
    bounds.y > -spanY && bounds.y < spanY + bounds.height;
...
export function resolveInitialWindowState(): InitialWindowState {
  ...
  if (!saved) {
    return {
      frame: centeredDefault(DEFAULT_WIDTH, DEFAULT_HEIGHT),
      maximized: true,
    };
  }
  ...
  if (saved.displays !== displayFingerprint() || !isSanePosition(sized)) {
    return {
      frame: centeredDefault(sized.width, sized.height),
